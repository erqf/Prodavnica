...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, where, getDocs, limit, doc } from 'firebase/firestore'; 
...
// ADMIN CONFIG
const sellers = [
    { id: 1, name: "Milena", avatar: "M", color: "from-purple-600 to-pink-600", username: "milena", password: "milena123", uid: null },
    { id: 2, name: "Anastasija", avatar: "A", color: "from-blue-600 to-purple-600", username: "anastasija", password: "anastasija123", uid: null },
    { id: 3, name: "Kostolacka Majka", avatar: "K", color: "from-green-600 to-teal-600", username: "kostolacka", password: "kostolacka123", uid: null },
    { id: 4, name: "Nemanja", avatar: "N", color: "from-orange-600 to-red-600", username: "nemanja", password: "nemanja123", uid: null }
];
...
const App = () => {
    ...
    // FUNKCIJA ZA POVEZIVANJE KORISNIKA I PRODAVCA
    const getSellerInfo = (user) => {
        if (!user) return { isSeller: false, data: null };
        const seller = sellers.find(s => s.username === user.username);
        return {
            isSeller: !!seller,
            data: seller
        };
    ...
    const sellerInfo = getSellerInfo(currentUser);
    const isSeller = sellerInfo.isSeller;
    const sellerData = sellerInfo.data;
    
    // Auth state listener
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (user) {
                const isUserSeller = user.email.startsWith('seller-');
                const username = isUserSeller 
                    ? user.email.split('@')[0].replace('seller-', '')
                    : user.email.split('@')[0];
                ...
                if (isUserSeller) {
                     const sellerIndex = sellers.findIndex(s => s.username === username);
                     if(sellerIndex !== -1) {
                        sellers[sellerIndex].uid = user.uid; 
                     }
                }
                
                console.log('✅ Uspesno logovan kao:', username); 
            } else {
                setCurrentUser(null);
    ...
    // Load volume settings (no change)
    useEffect(() => {
    ...
    // --- POPRAVLJENA LOGIKA INBOXA ZA PRODAVCA (KORISTI SERVER FILTER) ---
    useEffect(() => {
        console.log("🔥 Pokrenut Seller Inbox Listener Setup. Is Seller:", sellerInfo.isSeller, "Seller ID:", sellerInfo.data?.id);
        
        if (!sellerInfo.isSeller || !sellerInfo.data) {
            console.log("   --> Preskačem: Nije Prodavac ili nedostaju podaci.");
            return;
        }

        const currentSellerData = sellerInfo.data; 
        
        // NOVI UPIT: Filtriraj chatove na osnovu POLJA 'sellerId'
        const q = query(
            collection(db, 'chats'),
            where('sellerId', '==', currentSellerData.id) 
        );
            
        const unsubscribeAllChats = onSnapshot(q, async (chatsSnapshot) => {
            const newInbox = {};
            
            const processingPromises = chatsSnapshot.docs.map(async (chatDoc) => {
                const chatId = chatDoc.id; // Nasumični ID dokumenta
                const chatData = chatDoc.data(); // Podaci iz chat dokumenta (ima sellerId)
                
                // Dobavi poslednju poruku za taj chatKey
                ...
                if (messages.length > 0) {
                    ...
                    newInbox[chatId] = {
                        chatId,
                        buyerName: chatData.buyerName, // Koristi ime kupca iz chat dokumenta
            ...
            // KORISTI SE SPREAD OPERATOR ZA GARANTOVANO AZURIRANJE UI-a
            setSellerInbox({...newInbox}); 
            console.log("✅ Ažuriran Seller Inbox. Novi broj poruka:", Object.keys(newInbox).length); 

        }, (error) => {
            console.error("Greška pri učitavanju inboxa:", error);
        });
        
        return () => {
            unsubscribeAllChats();
        };
    }, [sellerInfo, currentUser, isInboxVisible]); 

    // --- REAL-TIME CHAT LISTENER (ZA KUPCA I PRODAVCA) ---
    useEffect(() => {
        if (!currentUser || (!selectedSeller && !selectedChat)) return;
        
        // Koristi se samo chatID koji je setovan u selectedChat
        let chatKey = selectedChat?.chatId;
        
        // Ako chatKey nije postavljen (npr. tek otvoren chat prozor kod Kupca), preskačemo slušanje
        if (!chatKey) return;
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const messages = snapshot.docs.map(doc => {
                const data = doc.data();
                const senderIsCurrentUser = data.senderUid === currentUser.uid;
                
                return {
                    text: data.text,
                    sender: senderIsCurrentUser ? 'user' : 'other', 
    ...
    const handleOpenInbox = () => {
        ...
        setShowSellerDashboard(prev => !prev);
        setSelectedSeller(null); 
        setSelectedChat(null);
    };

    // ✅ POPRAVLJENO: Odmah otvara prozor za Kupca, ID chata se rešava slanjem poruke
    const handleSelectSeller = (seller) => {
        setSelectedSeller(seller);
        setShowSellerModal(false);
        
        // Postavi sve potrebne podatke, ali ID chata je null dok poruka ne bude poslata
        setSelectedChat({ 
            chatId: null, 
            sellerId: seller.id, 
        ...
        // Resetuj chat za taj null ključ da prikaže prazan prozor (ako je potrebno)
        setChats(prev => ({ ...prev, [null]: [] }));
    };
...
// ✅ POPRAVLJENA FUNKCIJA SLANJA: Pronalazi ili kreira chat dokument u kolekciji "chats"
    const handleSendMessage = async () => {
        if (!chatMessage.trim() || !currentUser) return;

        let chatKey = selectedChat?.chatId;
        let targetSellerId = selectedSeller?.id || selectedChat?.sellerId; // Koristi selectedChat?.sellerId ako je Prodavac
        let isNewChat = false; 

        if (!targetSellerId) {
             console.error("Nedostaje ID prodavca za slanje poruke.");
             return;
        }

        try {
            // 1. POKUŠAJ PRONAĆI POSTOJEĆI CHAT DOKUMENT (Ako je Kupac i nema chatKey)
            if (!chatKey) {
                 const qFind = query(collection(db, 'chats'), 
                    where('buyerUid', '==', currentUser.uid),
                    where('sellerId', '==', targetSellerId),
                    limit(1)
                );
                const existingChat = await getDocs(qFind);
                
                if (!existingChat.empty) {
                    // Chat pronađen, koristimo njegov ID
                    chatKey = existingChat.docs[0].id;
                } else {
                    // Nije pronađen, moramo kreirati novi chat.
                    isNewChat = true;
                }
            }

            // 2. KREIRANJE NOVOG CHAT DOKUMENTA (Ako je isNewChat true)
            if (isNewChat) {
            ...
            // 3. Ažuriraj selectedChat sa novim chatKey-em
            setSelectedChat(prev => ({...prev, chatId: chatKey, sellerId: targetSellerId}));

            // 4. SLANJE PORUKE U PODKOLEKCIJU
            ...
            // Koristi alert za prikaz greške koju si ti video
            alert('Greška pri slanju poruke: ' + error.message); 
        }
    };

    // (Ostatak koda za volume, toggleMute, playAudio je bez promena)
    ...
    const currentChatKey = selectedChat?.chatId;
    // Ako Kupac otvori novi chat, currentChatKey je null, ali mi želimo da vidimo prazan prozor
    const currentChat = selectedChat && currentChatKey === null ? [] : (currentChatKey ? chats[currentChatKey] || [] : []);
        
    const unreadCount = Object.keys(sellerInbox).length;
        ...
        <div className="min-h-screen bg-zinc-950" onClick={playAudio}>
                                    ...
                                    {Object.values(sellerInbox)
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                ...
                                                {true && <Bell className="w-5 h-5 text-amber-600" />} 
                                            </div>
                                        </button>
                                    ))}
                                </div>
            ...
            {selectedChat && (
                <div className="fixed bottom-6 right-6 w-96 bg-zinc-900 rounded-lg border border-zinc-800 shadow-xl overflow-hidden z-50">
                    <div className={`bg-gradient-to-r ${selectedSeller?.color || 'from-zinc-800 to-zinc-700'} p-4 flex justify-between items-center`}>
                        <div className="flex items-center space-x-3">
